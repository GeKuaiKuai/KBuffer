...
ssize_t KBuffer::readFd(int fd)
{
    ...
    else if(static_cast<size_t>(t) <= writable)
    {
        ...
        {
            writestr_ = static_cast<DataBlock*>(vec[(t - (BLOCK_SIZE - writablePos_))/BLOCK_SIZE+1].iov_base);
...
int KBuffer::write(const void* buf,size_t size)
{
